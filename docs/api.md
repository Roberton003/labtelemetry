...
- Reading payloads include `source`, a short lineage field such as
  `simulator:seed=42`, `modbus:host:port`, or `opcua:opc.tcp://host:port`.

- Legacy bare API routes without `/api/` are not part of the public contract.
- The source health endpoint reports simulator, Modbus, and OPC-UA. It is
  operational metadata: it returns each adapter's last known state and does
  not open a live connection per request.
